...
//    }
    ...
    protected String doInBackground(String... params) {
    	 String queryS = "PREFIX dcterms: <http://purl.org/dc/terms/> "
    	 		+ "PREFIX health: <http://mobilehealth.tw.rpi.edu/ontology/health.ttl#> "
    	 		+ "SELECT ?date ?value where { "
    	 		+ "?s health:ofCharacteristic health:Glucose . "
    	 		+ "?s dcterms:date ?date . "
    	 		+ "?s health:hasValue ?value . }";
    ...
    protected void onPostExecute(String result) {
    	...
    	while (resultSet.hasNext())
    	    {
    	    	...
    	    	String date = next.get("date").toString();
    	    	String value = next.get("value").toString();
    	    	DateTime datet = fmt.parseDateTime(date.split("\\^")[0]);	 
    	   String 	date2 = String.valueOf(datet.getYear()) + "," + String.valueOf(datet.getMonthOfYear()-1) +"," + String.valueOf(datet.getDayOfMonth());

    	    	builder1.append("[Date.UTC(" + date2+"),"+value.split("\\^")[0]+"],");

    	    }
	    	Log.e("date", builder1.toString());
     ...
     // prepareListData(resultSet);
      ...
      //expListView.setAdapter(listAdapter);
    	...
    	String content = "<html><body><h1>Loading</h1></body></html>";
    	StringBuilder builder = new StringBuilder();

    	try {
    		builder.append(readFileAsString("highcharts-part1.html"));
    		builder.append(readFileAsString("highcharts-part2.html"));
    		builder.append(builder1.toString());
    		builder.append(readFileAsString("highcharts-part3.html"));
    		content = builder.toString();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    	//content = "<html><body><h1>Loading</h1></body></html>";
    	webView.loadDataWithBaseURL("http://code.highcharts.com/", content, "text/html", null, "http://code.highcharts.com/");
